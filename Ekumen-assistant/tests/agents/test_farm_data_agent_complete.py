"""
Test Farm Data Agent - Complete 10/10 Production-Ready Implementation
...
def test_sophisticated_prompt():
    ...
    # Get the prompt
    prompt = get_farm_data_react_prompt(include_examples=False)
...
def test_token_optimization():
    ...
    prompt_without = get_farm_data_react_prompt(include_examples=False)
    prompt_with = get_farm_data_react_prompt(include_examples=True)
...
def test_metrics_tracking():
    ...
    agent = FarmDataIntelligenceAgent(enable_metrics=True)
    
    # Simulate some calls
    agent._update_metrics(success=True, iterations=3, tools_used=["get_farm_data", "calculate_performance_metrics"])
    agent._update_metrics(success=True, iterations=5, tools_used=["get_farm_data"])
    ...
    assert 'get_farm_data' in metrics['tool_usage'], "Should track tool usage"
    assert metrics['tool_usage']['get_farm_data'] == 2, "Should count tool usage correctly"
...
def run_all_tests():
    """Run all tests"""
    print("\n" + "="*80)
    print("FARM DATA AGENT - COMPLETE 10/10 PRODUCTION-READY TESTS")
    print("="*80)
