"""
Simple Sustainability Intelligence Agent - Uses LangChain directly without broken base classes.
...
class SustainabilityIntelligenceAgent:
    """
    Sustainability Intelligence Agent using 4 production-ready tools.
    
    Simple wrapper around LangChain's ReAct agent that:
    - Holds reference to production tools
    - Provides sustainability-specific prompt
    ...
    def __init__(
        ...
        """
        Initialize Sustainability Intelligence Agent.
        
        Args:
            llm: Language model to use (if None, creates default ChatOpenAI)
            tools: List of tools to use (if None, uses 4 production sustainability tools)
        ...
        logger.info(f"Initialized Sustainability Intelligence Agent with {len(self.tools)} production tools")
    
    def _create_agent(self):
        """Create LangChain ReAct agent with sustainability-specific prompt."""
        prompt = self._get_prompt_template()
        return create_react_agent(self.llm, self.tools, prompt)
    
    def _get_prompt_template(self) -> PromptTemplate:
        """Get sustainability-specific prompt template for ReAct agent."""
        template = """Tu es un expert en agriculture durable et agroenvironnement français.
...
EXPERTISE:
- Calcul d'empreinte carbone (émissions GES, séquestration, bilan net)
- Évaluation de la biodiversité (7 indicateurs: haies, prairies, rotation, etc.)
- Analyse de santé des sols (matière organique, structure, vie biologique)
- Gestion de l'eau (efficience irrigation, qualité, impact économique)

INSTRUCTIONS:
1. Identifie les aspects de durabilité à évaluer
2. Utilise les outils appropriés pour analyser les pratiques
3. Fournis des recommandations concrètes pour améliorer la durabilité
4. Quantifie les impacts environnementaux et économiques
5. Réponds toujours en français avec un ton professionnel et encourageant

IMPORTANT:
- Valorise les bonnes pratiques existantes
- Propose des améliorations progressives et réalistes
- Quantifie les bénéfices environnementaux ET économiques
- Prends en compte les contraintes de l'exploitation
- Encourage la transition agroécologique
...
{agent_scratchpad}"""
    ...
    def _format_result(self, result: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_type": "sustainability_intelligence",
    ...
    async def aprocess(self, message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Sustainability Agent processing: {message[:100]}...")
            ...
            logger.warning(f"Validation error in Sustainability Agent: {e}")
            return {
            ...
            # Unexpected errors
            logger.error(f"Unexpected error in Sustainability Agent: {e}", exc_info=True)
            return {
    ...
    def process(self, message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Sustainability Agent processing (sync): {message[:100]}...")
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "focus_areas": ["climate", "biodiversity", "soil", "water"],
            "language": "french"
        }
