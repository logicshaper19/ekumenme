...
class PlanningIntelligenceAgent:
    ...
    def __init__(
        ...
        """
        Initialize Planning Intelligence Agent with sophisticated prompts.
        ...
        logger.info(
            f"✅ Planning Intelligence Agent initialized: "
    ...
    def _create_agent(self):
        """Create LangChain ReAct agent with sophisticated planning prompt."""
        prompt = self._get_prompt_template()
        return create_react_agent(self.llm, self.tools, prompt)

    def _get_prompt_template(self) -> ChatPromptTemplate:
        """
        Get sophisticated planning-specific prompt template for ReAct agent.

        Uses the centralized prompt system from app/prompts/Planning_prompts.py
        which includes:
        ...
        Returns:
            ChatPromptTemplate configured for planning intelligence
        """
        # Get sophisticated prompt from centralized prompt system
        return get_planning_react_prompt(include_examples=self.enable_dynamic_examples)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        Example:
            >>> agent.get_metrics()
            {
                ...
                'success_rate': 80.0,
                'avg_iterations': 3.5,
                'tool_usage': {'get_Planning': 8, 'calculate_disease_identification': 5},
    ...
    def reset_metrics(self):
        ...
        if self.enable_metrics:
            ...
            logger.info("Planning Agent metrics reset")
