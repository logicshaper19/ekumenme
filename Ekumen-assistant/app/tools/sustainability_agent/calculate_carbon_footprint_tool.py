...
async def calculate_carbon_footprint(
    ...
    try:
        ...
        # Unexpected error
        logger.error(f"Unexpected error in calculate_carbon_footprint: {e}", exc_info=True)
        error_result = CarbonFootprintOutput(
            success=False,
            crop=crop,
            surface_ha=surface_ha,
...
calculate_carbon_footprint_tool = StructuredTool.from_function(
...
NE REMPLACE PAS bilan carbone certifié.""",
    ...
    return_direct=False,
    coroutine=calculate_carbon_footprint,
