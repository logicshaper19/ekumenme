...
async def assess_biodiversity(
    ...
    try:
        ...
        # Unexpected error
        logger.error(f"Unexpected error in assess_biodiversity: {e}", exc_info=True)
        error_result = BiodiversityOutput(
            success=False,
            surface_ha=surface_ha,
            overall_biodiversity_score=0.0,
...
assess_biodiversity_tool = StructuredTool.from_function(
    func=assess_biodiversity,
