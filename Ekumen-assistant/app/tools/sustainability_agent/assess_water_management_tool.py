...
async def assess_water_management(
    surface_ha: float,
    crop: str,
    ...
    try:
        ...
        # Unexpected error
        logger.error(f"Unexpected error in assess_water_management: {e}", exc_info=True)
        error_result = WaterManagementOutput(
            success=False,
            surface_ha=surface_ha,
            crop=crop,
...
assess_water_management_tool = StructuredTool.from_function(
    func=assess_water_management,
...
Aide optimiser ressource eau, réduire coûts, améliorer durabilité.""",
    ...
    return_direct=False,
    coroutine=assess_water_management,
