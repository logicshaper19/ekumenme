...
class EnhancedSoilHealthService:
    """
...
async def analyze_soil_health_enhanced(
    ...
    try:
        ...
        # Execute service
        service = EnhancedSoilHealthService()
        ...
        # Unexpected error
        logger.error(f"Unexpected error in analyze_soil_health_enhanced: {e}", exc_info=True)
        error_result = SoilHealthOutput(
            success=False,
            overall_score=0.0,
            overall_status=SoilHealthStatus.CRITICAL,
...
analyze_soil_health_tool_enhanced = StructuredTool.from_function(
    func=analyze_soil_health_enhanced,
...
Retourne analyse détaillée avec actions concrètes.""",
    ...
    return_direct=False,
    coroutine=analyze_soil_health_enhanced,
