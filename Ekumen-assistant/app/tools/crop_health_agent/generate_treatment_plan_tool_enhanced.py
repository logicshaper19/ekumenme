"""
Enhanced Treatment Plan Generation Tool

Integrates with Phase 2 database and enhanced crop health tools for:
- Comprehensive treatment planning
- Integration with disease, pest, and nutrient analyses
- Crop-specific recommendations using Crop model
- Cost analysis
- Monitoring plans
- Pydantic validation
- Redis caching
...
class GenerateTreatmentPlanToolEnhanced(BaseTool):
    """
    Enhanced treatment plan generation tool with Phase 2 database integration.
    ...
    name: str = "generate_treatment_plan"
    description: str = "Génère un plan de traitement complet intégrant maladies, ravageurs et carences"
    
    @redis_cache(ttl=1800, category="crop_health")
    async def execute(self, input_data: TreatmentPlanInput) -> TreatmentPlanOutput:
        """
    ...
    def _run(self, **kwargs) -> str:
        ...
        try:
            input_data = TreatmentPlanInput(**kwargs)
            ...
            error_output = TreatmentPlanOutput(
                success=False,
...
generate_treatment_plan_tool_enhanced = GenerateTreatmentPlanToolEnhanced()
